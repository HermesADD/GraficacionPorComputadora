class GenericGeometry {
  ...
  createFlatVAO(gl) {
    ...
    //////////////////////////////////////////////////
    if ((this.getUVCoordinates) && (this.material.getAttribute("a_texcoord") != undefined)) {
      let uv = this.getUVCoordinates(vertices);
      let uvBuffer = gl.createBuffer();
      gl.bindBuffer(gl.ARRAY_BUFFER, uvBuffer);
      gl.bufferData(gl.ARRAY_BUFFER, new Float32Array(uv), gl.STATIC_DRAW);
      gl.enableVertexAttribArray(this.material.getAttribute("a_texcoord"));
  ...
  /**
   */
  draw(gl, projectionMatrix, viewMatrix, lights) {
    ...
    for (let i=0; i<lights.length; i++) {
      // u_light[i].position
      if (this.material.getUniform("u_light["+i+"].position") != undefined) {
        gl.uniform3fv(this.material.getUniform("u_light["+i+"].position"), lights[i].getPosition());
      }
      // u_light["+i+"].La
      if (this.material.getUniform("u_light["+i+"].La") != undefined) {
        gl.uniform3fv(this.material.getUniform("u_light.["+i+"]La"), lights[i].ambient);
      }
      // u_light["+i+"].Ld
      if (this.material.getUniform("u_light["+i+"].Ld") != undefined) {
        gl.uniform3fv(this.material.getUniform("u_light["+i+"].Ld"), lights[i].diffuse);
      }
      // u_light["+i+"].Ls
      if (this.material.getUniform("u_light["+i+"].Ls") != undefined) {
        gl.uniform3fv(this.material.getUniform("u_light["+i+"].Ls"), lights[i].especular);
      }
      // u_light["+i+"].color
      if (this.material.getUniform("u_light["+i+"].color") != undefined) {
        gl.uniform3fv(this.material.getUniform("u_light["+i+"].color"), lights[i].color);
      }
    }
    ////////////////////////////////////////////////////////////
    ...
    // PBR estándar
    // u_albedo
    if (this.material.getUniform("u_albedo") != undefined) {
      gl.uniform3fv(this.material.getUniform("u_albedo"), this.material.color);
    }
    // u_metallic
    if (this.material.getUniform("u_metallic") != undefined) {
      gl.uniform1f(this.material.getUniform("u_metallic"), this.material.metallic);
    }
    // u_roughness
    if (this.material.getUniform("u_roughness") != undefined) {
      gl.uniform1f(this.material.getUniform("u_roughness"), this.material.roughness);
    }
    // u_ao
    if (this.material.getUniform("u_ao") != undefined) {
      gl.uniform1f(this.material.getUniform("u_ao"), this.material.ao);
    }
    ...
    // PBR texturas
    // u_albedoMap
    if (this.material.getUniform("u_albedoMap") != undefined) {
      gl.activeTexture(gl.TEXTURE0);
      gl.bindTexture(gl.TEXTURE_2D, this.material.albedoTexture);
      gl.uniform1i(this.material.getUniform("u_albedoMap"), 0);
    }
    // u_normalMap
    if (this.material.getUniform("u_normalMap") != undefined) {
      gl.activeTexture(gl.TEXTURE1);
      gl.bindTexture(gl.TEXTURE_2D, this.material.normalTexture);
      gl.uniform1i(this.material.getUniform("u_normalMap"), 1);
    }
    // u_metallicMap
    if (this.material.getUniform("u_metallicMap") != undefined) {
      gl.activeTexture(gl.TEXTURE2);
      gl.bindTexture(gl.TEXTURE_2D, this.material.metallicTexture);
      gl.uniform1i(this.material.getUniform("u_metallicMap"), 2);
    }
    // u_metallicMap
    if (this.material.getUniform("u_roughnessMap") != undefined) {
      gl.activeTexture(gl.TEXTURE3);
      gl.bindTexture(gl.TEXTURE_2D, this.material.roughnessTexture);
      gl.uniform1i(this.material.getUniform("u_roughnessMap"), 3);
    }
    // u_metallicMap
    if (this.material.getUniform("u_aoMap") != undefined) {
      gl.activeTexture(gl.TEXTURE4);
      gl.bindTexture(gl.TEXTURE_2D, this.material.ambientOcclusionTexture);
      gl.uniform1i(this.material.getUniform("u_aoMap"), 4);
    }
    ////////////////////////////////////////////////////////////
