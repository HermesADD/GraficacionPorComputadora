...
window.addEventListener("load", async function(evt) {
  ...
  let grass_color = await loadImage("grass01.jpg");
  let grass_normal = await loadImage("grass01_n.jpg");

  ////////////////////////////////////////////////////
  let geometry = [
    new Plano(
      gl,
      //new TextureMaterial(gl, grass_color),
      new TextureNormalMaterial(gl, grass_color, grass_normal),
      translate(0, 0, 0)
    ),
    new Plano(
      gl,
      new TextureNormalMaterial(gl, tex_color, text_normal),
      multiply(rotateX(Math.PI/2),translate(0,-1,-1))
    )
  ];
