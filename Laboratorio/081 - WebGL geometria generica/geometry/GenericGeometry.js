class GenericGeometry {
  ...
  init(gl) {
    ...
    // Solo si esta definida la función getFaces entonces es posible construir la geometría suave
    if (this.getFaces) {
  ...
  createSmoothVAO(gl, positionAttributeLocation, normalAttributeLocation) {
    ...
    this.smooth_normal_geometry.transform = this.transform;
  }
  ...
  createFlatVAO(gl, positionAttributeLocation, normalAttributeLocation) {
    let vertices;
    // Si existe el arreglo de caras entonces hay que desempaquetar la geometría suave para obtener la versión plana
    if (this.faces) {
    ...
    this.flat_normal_geometry.transform = this.transform;
  }
  ...
  draw(gl, projectionMatrix, viewMatrix, lightPosition) {
    ...
    gl.uniformMatrix4fv(this.VM_matrixLocation.toArray(), true, viewModelMatrix.toArray());
    ...
    // Smooth shading
    // se agrega la condición de que tenga la función getFaces para que los objetos geométricos que no sean suaves se dibujen sin problemas
