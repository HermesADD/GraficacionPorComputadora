class GenericGeometry {
  ...
  init(gl) {
    ...
    this.simpleMaterial = new SimpleMaterial(gl);
  }
  ...
  /**
   */
  draw(gl, projectionMatrix, viewMatrix, light, projectedTexture, textureMatrix) {
    ...
    // u_M_matrix
    if (this.material.getUniform("u_M_matrix")) {
    ...
    // u_texture_matrix
    if (this.material.getUniform("u_texture_matrix")) {
      gl.uniformMatrix4fv(this.material.getUniform("u_texture_matrix"), true, textureMatrix);
    }
    ...
    //
    if (projectedTexture && this.material.getUniform("u_projected_texture")) {
      gl.activeTexture(gl.TEXTURE0);
      gl.bindTexture(gl.TEXTURE_2D, projectedTexture);
      gl.uniform1i(this.material.getUniform("u_projected_texture"), 0);
    }
    ////////////////////////////////////////////////////////////
  ...
  drawSimple(gl, textureProjectionMatrix) {
    gl.useProgram(this.simpleMaterial.program);

    // u_PVM_matrix
    if (this.simpleMaterial.getUniform("u_PVM_matrix") != undefined) {
      gl.uniformMatrix4fv(this.simpleMaterial.getUniform("u_PVM_matrix"), true, multiply(textureProjectionMatrix, this.transform));
    }

     // Smooth shading
     if (this.isSmooth && this.getFaces) {
      gl.bindBuffer(gl.ARRAY_BUFFER, this.smooth_positionBuffer);
      gl.enableVertexAttribArray(this.simpleMaterial.getAttribute("a_position"));
      gl.vertexAttribPointer(this.simpleMaterial.getAttribute("a_position"), 3, gl.FLOAT, false, 0, 0);
